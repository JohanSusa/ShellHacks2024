...
def st_horizontal():
    ...
    with st.container():
        ...
        # Instead of yielding, return the container itself
        return st.container()
...
with st.container():  # Enclose both sections in a single container
    ...
    if st.session_state.current_question_index < len(questions):
        ...
        # Button interaction with callback and horizontal layout
        with st.chat_message("user"):
            # Get the container from st_horizontal and use it for the buttons
            button_container = st_horizontal()
            for option in options:
                button_container.button(option, key=f"button_{option}", on_click=handle_button_click, args=(option,))
...
# Option to restart the quiz (outside the main container)
